...
into the wiki under each one.

### What happened to the relics

Those five photographs used to be the payload of the relics: ten stickers
hidden across the front door and the main floor, each opening a panel with a
line, a date and a link, with a found-state counter following you between the
two pages. The system is gone — `Relic`, `RelicArt`, `RelicCase`,
`src/content/relics.ts` and `src/state/relics.ts` with it.

The writing is not gone. It is under the photographs at a size you can read,
which is where it should have been: a hidden thing that nobody finds is not a
thing on the site, and five photographs of the only person who has read the
whole wiki were the wrong thing to have turned into a pixel hunt. The five
relics that had no photograph are not carried over — their material is all in
the wiki in longer form, which is where every one of these links to anyway.

### `<Plate>`

One component (`src/components/Plate.tsx`) draws both groups. Nothing on this
site is cut square — the punk pass is four devices and one of them is called
THE SCISSORS — so every plate is clipped to a torn polygon (`torn`, `shard`,
`rip`, `slab`). `clip-path` is a paint-time operation on a static element: it
costs nothing per frame and gives an edge that looks handled rather than an
outline that looks masked. `kiss-uniform` is the exception and takes `none`: it
was drawn on flat white, so a flood fill inward from the border gave it a real
alpha channel, and a box would put back the rectangle the knockout removes.

No colour grading is baked into any file. A halftone screen and the room's own
colour go over the top in CSS, so one file reads as neon in VOID, as sodium in
LOVE HOTEL and as a second-generation photocopy in RIOT.

### The bug this shipped with once

The first version wrote `src: '/art/kiss-neon.webp'` — an absolute path, which
is correct on a site served from a domain root and wrong on this one. This
deploys to a **subpath**, `caakehorn.github.io/home/`, with `base: '/home/'`.

Vite rewrites `url()` references inside CSS when you set that. It does not
rewrite string literals inside TypeScript, because it cannot know a string is a
URL. Every plate referenced from CSS resolved; every plate in an `<img src>`
asked for `/art/…` instead of `/home/art/…` and got a 404. **Every image on the
site was broken in production and fine in every local check**, because
`vite preview` serves from the root — which this file already warned about
under Deployment, in the paragraph immediately above where it was ignored.

`src/content/art.ts` builds every path through `import.meta.env.BASE_URL`, and
the check that catches this class of mistake is to build with `BASE_PATH` set
and serve `dist/` under that prefix with a 404 fallback. Not `vite preview`.

`public/art/README.md` and `public/ally/README.md` carry the file tables, the
crops, and the one thing worth saying out loud about putting photographs of a
