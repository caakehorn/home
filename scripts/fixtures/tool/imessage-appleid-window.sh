...
OUT="$DESK/imessage-someone-icloud-com-$STAMP.txt"
MANIFEST="$DESK/imessage-someone-icloud-com-$STAMP.manifest.txt"
...
SOURCE_EXPECT="$(/usr/bin/sqlite3 "$DB" <<'SQL_SOURCE_COUNT'
...
SNAP_EXPECT="$(/usr/bin/sqlite3 "$WORK/chat.db" <<'SQL_SNAPSHOT_COUNT'
SELECT count(*) FROM (SELECT m.ROWID
FROM message m
LEFT JOIN handle h ON h.ROWID = m.handle_id
-- One chat per message, chosen deterministically. A message in two chats would
...
PYSRC
  ...
  STATS="$("$PY" "$WORK/decode.py" 'txt' "$WORK/raw.csv" "$OUT" 2>&1 >/dev/null)"
...

{
  ...
  say "from           $DB"
  say "messages       $WROTE"
  say "who            SomeOne@iCloud.com"
  say "scope          whole conversations, including group threads"
  say "window         2020-01-01 to 2020-12-31 inclusive, in your Mac's own timezone"
